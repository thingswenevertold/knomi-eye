...
def render_frame(t, eye_open, twitch, gaze_x, gaze_y, sleepy=False):
    ...
    for side in (-1, 1):
        ...
        ery = 0.100 * eye_open
        sclera = mask_ellipse(ecx, eye_y, 0.112, ery)
...
"""
    ...
    lines.append("")
    lines.append("const asciiart::Frame CAT_ASCII[] = {")
    for i in range(len(awake)):
        lines.append("    { CAT_%02d, %d }," % (i, ROWS))
    lines.append("};")
    lines.append("const int CAT_ASCII_COUNT = %d;" % len(awake))
    lines.append("")
    lines.append("const asciiart::Frame CAT_ASCII_SLEEP[] = {")
    for i in range(len(sleep)):
        lines.append("    { NAP_%02d, %d }," % (i, ROWS))
    lines.append("};")
    lines.append("const int CAT_ASCII_SLEEP_COUNT = %d;" % len(sleep))
    lines.append("")
    lines.append("}")
    lines.append("")
...
if __name__ == "__main__":
    ...
    sleep = build_sleep()
    emit(awake, sleep)
    print("frames eveille : %d   sommeil : %d" % (len(awake), len(sleep)))
    print("grille : %dx%d, rampe de %d niveaux" % (COLS, ROWS, len(RAMP)))
    print()
    print("apercu (frame 0) :")
    for r in awake[0]:
        print("  |" + r + "|")
